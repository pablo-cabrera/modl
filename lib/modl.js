(function (node) {
     ...
     * @final
     */
    var MODULE_REFERENCE = ilk.Token.create();
    ...
    var Ready = tokens(function (
        ...
        proto({
            ...
            ready: that(function (t) {
                ...
                parts.forEach(callbacks, function (f) { f(); });
            }),
            ...
            fail: that(function (t, reason) {
                ...
                t[readyCallbacks] = null;
                t[failCallbacks] = null;
                parts.forEach(callbacks, function (f) {
                    try {
                        f(reason);
                    } catch (e) {
     ...
     */
    var Asset = tokens(function (myName) {
    ...
    var loadAsset = function (a, s, f) {
        // console.log("loadAsset", a);
        if (parts.hop(hydratedAssets, a)) {
    ...
    var loadScript = node?
        ...
        (function () {
             ...
             */
            var track = (function () {
                if ((function (m) { return m && m[1]; })
                        (navigator.userAgent.match(/MSIE (8|9).0([^;]*)/))) {

                    return (function () {
                        var check = function (readyState, context) {
                                if ("loaded" === readyState ||
                                        "complete" === readyState) {
                            ...
                            hijack = function (name, context) {
                                return parts.args(function (args) {
                                    ...
                                    parts.merge(modl, context.previousMethods);
    ...
    var Module = tokens(function (
        ...
        return Ready.descend(that(function (t, name, path, parent, hydrating) {
            ...
            console.log("Creating module " + name + (parent?
                    " with parent " + parent[myName]: " without parent"));
        ...
        proto(hydrateModule, that(function (that, name, value, root) {
            var module, jsonReference, pieces;
            ...
            if (typeof value === "object") {
                ...
                jsonReference = root;
                pieces = value.split("/");
                if (pieces.length === 1 && parts[0] === "") {
                    module = root[MODULE_REFERENCE];
                } else {
                    parts.forEach(pieces, function (p) {
        ...
        proto(hydrateModuleReferences, that(function (that, json, root, stack) {
            ...
            parts.forEach(json, function (v, k) {
                if (parts.isString(v)) {
                    var ref = root,
                        pieces = v.split("/"),
                        module;
                    ...
                    if (pieces.length === 1 && parts[0] === "") {
                        ...
                        parts.forEach(pieces, function (p) { ref = ref[p]; });
                    ...
                    v[MODULE_REFERENCE]
                            [hydrateModuleReferences](v, root, stack);
                }
            });
        })).
        ...
        proto({
             ...
             * @param {function} fail The fail callback
             */
            module: that(function (t, name, success, fail) {
                var path, module;

                console.log("Loading module " + name);
                console.log("Current module " + t[myName]);


                if (name in t[modules]) {
                    module = t[modules][name];
                    ...
                    path = Module.path(t[myPath], name);

                    module = new Module(name, path, t);
                    ...
                    module.whenFail(function () {
                        console.log("Failed loading module " + name +
                                " looking up for parent for " + t[myName]);

                        var parent = t[myParent];
                        currentModule = t;
            ...
            hydrate: that(function (t, json, root) {
                ...
                parts.forEach(json, function (value, name) {
    ...
    var Loader = tokens(function (
        ...
        proto(loadUses, that(function (that, success, fail) {
            var myUses = parts.map(that[uses], function (v, k) { return k; }),
                loadedUses = {},
